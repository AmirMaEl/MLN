...
class VQVAE(nn.Module):
    ...
    def fhat_to_img(self, f_hat: torch.Tensor):
        f_hat = f_hat.to(self.quant_conv.weight.dtype)
        return self.decoder(self.post_quant_conv(f_hat)).clamp_(-1, 1)
    def to_cpu(self):
        self.encoder.to('cpu')
        self.decoder.to('cpu')
        self.quant_conv.to('cpu')
        self.post_quant_conv.to('cpu')
        self.quantize.to('cpu')
    ...
    def img_to_fhat(
        ...
        f = self.quant_conv(self.encoder(inp_img_no_grad))
        return self.quantize.f_to_idxBl_or_fhat(
            f, to_fhat=True, v_patch_nums=v_patch_nums, noise_std=noise_std,step=step
        )['idx_list']
    def img_to_frest(
        self,
        inp_img_no_grad: torch.Tensor,
    ...
    def to_fhat(
        ...
        f = self.fhat_to_img(f) 
        f = self.quant_conv(self.encoder(f))
        return self.quantize.finit_to_fhat(
            f, to_fhat=True, v_patch_nums=v_patch_nums, noise_std=noise_std,step=step, f_init=f_init
        )
        ...
        return [
            f_hat
                for f_hat in self.quantize.embed_to_fhat(
                    ms_h_BChw, all_to_max_scale=same_shape, last_one=False
                )
            ]
    ...
    def img_to_fhat_single(
        self,
        x,
        v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None,
    ) -> torch.Tensor:
        f_hat, usages, vq_loss = self.quantize(
            self.quant_conv(self.encoder(x)), ret_usages=True
        )
