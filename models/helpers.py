...
def deterministic_gumbel_softmax(logits, tau=1.0, hard=False, eps=1e-10, dim=-1):
    """
    A deterministic implementation of the Gumbel softmax.
    Ensuring determinism relies on the fact that `torch.rand_like` uses the global seed.
    """
    # Sample from U(0, 1)
    u = torch.rand_like(logits)
    # Transform U to Gumbel noise: -log(-log(U))
    gumbels = -torch.log(-torch.log(u + eps) + eps)
    # Add noise to the logits and scale by temperature
    y = logits + gumbels
    y_soft = torch.softmax(y / tau, dim=dim)
    
    if hard:
        # Convert soft probabilities to one-hot vectors
